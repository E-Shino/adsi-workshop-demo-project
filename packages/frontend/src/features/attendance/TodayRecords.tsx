  ...
  return (
    <div className="rounded-lg border p-6">
      ...
      <div className="space-y-2">
        {records.map((record) => (
          <div key={record.id} className="py-1.5 border-b last:border-b-0">
            ...
            {(record.clockInMemo || record.clockOutMemo) && (
              <div className="mt-1 text-xs text-muted-foreground flex gap-4">
                {record.clockInMemo && <span>出勤: {record.clockInMemo}</span>}
                {record.clockOutMemo && <span>退勤: {record.clockOutMemo}</span>}
              </div>
            )}
          </div>
        ))}
